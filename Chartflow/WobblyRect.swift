...
struct WobblyBoxBackground: View {
    ...
    @AppStorage("boxMovementEffect") private var effectRawValue = BoxMovementEffect.random.rawValue
    ...
    var body: some View {
        ...
        case .random:
            TimelineView(.periodic(from: .now, by: 0.12)) { timeline in
                let phase = floor(CGFloat(timeline.date.timeIntervalSinceReferenceDate) * 8)
                animatedBox(phase: phase, wobble: wobble * 1.8, detail: 5)
            }
        case .flow:
            TimelineView(.animation) { timeline in
                let phase = CGFloat(timeline.date.timeIntervalSinceReferenceDate) * 2.2
                animatedBox(phase: phase, wobble: wobble, detail: 0)
            }
        case .none:
    ...
    private func animatedBox(phase: CGFloat, wobble: CGFloat, detail: Int) -> some View {
        WobblyRectangle(cornerRadius: cornerRadius, wobble: wobble, phase: phase, detail: detail)
            .fill(fillColor)
            .overlay(
                WobblyRectangle(cornerRadius: cornerRadius, wobble: wobble, phase: phase, detail: detail)
                    .stroke(strokeColor, lineWidth: lineWidth)
...
struct WobblyRectangle: Shape {
    ...
    private func detailedPath(in rect: CGRect) -> Path {
        ...
        for step in 0...steps {
            ...
            appendPoint(CGPoint(x: r + (w - 2 * r) * t, y: 0))
        }

        for step in 1...steps {
            let angle = -.pi / 2 + (CGFloat(step) / CGFloat(steps)) * .pi / 2
            appendPoint(CGPoint(x: w - r + cos(angle) * r, y: r + sin(angle) * r))
        }

        for step in 1...steps {
            let t = CGFloat(step) / CGFloat(steps)
            appendPoint(CGPoint(x: w, y: r + (h - 2 * r) * t))
        }

        for step in 1...steps {
            let angle = (CGFloat(step) / CGFloat(steps)) * .pi / 2
            appendPoint(CGPoint(x: w - r + cos(angle) * r, y: h - r + sin(angle) * r))
        }

        for step in 1...steps {
            let t = CGFloat(step) / CGFloat(steps)
            appendPoint(CGPoint(x: w - r - (w - 2 * r) * t, y: h))
        }

        for step in 1...steps {
            let angle = .pi / 2 + (CGFloat(step) / CGFloat(steps)) * .pi / 2
            appendPoint(CGPoint(x: r + cos(angle) * r, y: h - r + sin(angle) * r))
        }

        for step in 1...steps {
            let t = CGFloat(step) / CGFloat(steps)
            appendPoint(CGPoint(x: 0, y: h - r - (h - 2 * r) * t))
        }

        for step in 1...steps {
            let angle = .pi + (CGFloat(step) / CGFloat(steps)) * .pi / 2
            appendPoint(CGPoint(x: r + cos(angle) * r, y: r + sin(angle) * r))
        }
